...
const blank = () => ({ ad_pages: [], ad_page_revisions: [], site_settings: [], content_submissions: [], review_publications: [], trainers: [], trainer_pages: [], trainer_page_versions: [], portal_users: [{ ...officeUser }], send_to_live_log: [] });
...
function applyFilters(rows, params) {
  ...
  for (const [key, raw] of params) {
    ...
    if (op === "in") { const set = value.replace(/^\(|\)$/g, "").split(",").map(v => decodeURIComponent(v.replace(/^"|"$/g, ""))); out = out.filter(r => set.includes(String(r[key]))); }
  }
...
async function fakeSupabase(url, options = {}) {
  ...
  if (method === "POST") {
    ...
    const rows = (Array.isArray(body) ? body : [body]).map(row => ({ id: randomUUID(), created_at: new Date().toISOString(), updated_at: new Date().toISOString(), draft_revision: 1, published_revision: 0, published_at: null, ...(table === "ad_pages" ? { page_type: "ad", title: "" } : {}), ...row }));
...
pagesManifest.deps.fetch = fakeSupabase;
sitemap.deps.fetch = fakeSupabase;
...
const server = createServer(async (req, res) => {
  ...
  if (path === "/api/ad-pages" || path === "/api/pages") return call(adPages, url.searchParams);
  if (path === "/api/pages-manifest") return call(pagesManifest, url.searchParams, { forceAuth: false });
  if (path === "/api/sitemap" || path === "/sitemap.xml") return call(sitemap, url.searchParams, { forceAuth: false });
  const pSlug = path.match(/^\/p\/([^/]+)$/);
  if (pSlug) return call(adPage, new URLSearchParams({ slug: pSlug[1], via: "site", preview: url.searchParams.get("preview") || "" }), { forceAuth: url.searchParams.get("preview") === "1" });
  ...
  // site-builder: what middleware.js does on Vercel — a PUBLISHED site/landing page wins over the static file at its clean path.
  const cleanSlug = path.match(/^\/([a-z0-9-]{2,80})$/)?.[1];
  if (cleanSlug && url.searchParams.get("ldtt_static") !== "1") {
    const schema = process.env.LDTT_SANDBOX === "1" ? "practice" : "public";
    const hit = schemas[schema].ad_pages.find(r => r.slug === cleanSlug && r.status === "published" && ["site", "landing"].includes(r.page_type));
    if (hit) return call(adPage, new URLSearchParams({ slug: cleanSlug, via: "site" }), { forceAuth: false });
  }
  // static + cleanUrls
