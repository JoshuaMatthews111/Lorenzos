...
function renderView() {
  ...
  // release 2026-09-05: the practice copy cannot save a password (logins are
  // shared with live), so the two forced Settings gates are skipped there —
  // otherwise a real staff login whose account still carries
  // must_change_password (24 active rows on 2026-09-05) would be pinned to
  // Settings on every redraw with no way out. Live behaves exactly as before.
  if (!window.LDTT_IS_SANDBOX && portalUser?.must_change_password) {
    state.activeView = "settings";
  }
  if (!window.LDTT_IS_SANDBOX && portalProfileNeedsCompletion()) {
    state.activeView = "settings";
  }
...
async function savePipelineSettings() {
  ...
  const response = await fetch("/api/pipeline", {
    method: "POST", cache: "no-store",
    headers: { "Content-Type": "application/json", Authorization: `Bearer ${token || ""}` },
    body: JSON.stringify({ op: "save_settings", recipients, practice_trainer_phone: practicePhone, practice_operations_phone: opsPhone, practice_email_to: practiceEmail })
  });
  const payload = await response.json().catch(() => ({}));
