...
async function portalResendConfig(env = process.env) {
  ...
  try {
    const rows = await (await fetch(`${url}/rest/v1/communications_settings?select=setting_key,secret_id,value&setting_key=in.(resend_api_key,resend_from_address)`, { headers })).json();
    ...
    const secret = await (await fetch(`${url}/rest/v1/rpc/communications_read_setting_secret`, { method: "POST", headers, body: JSON.stringify({ p_key: "resend_api_key" }) })).json();
