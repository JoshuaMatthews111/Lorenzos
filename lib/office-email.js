...
// The portal already has a Resend key + from-address saved in Communications Settings (secret in the
// Supabase vault, read with rpc communications_read_setting_secret) - the same path api/communications.js
// and api/portal-password-reset.js use. Use it whenever no RESEND_API_KEY is set (Joshua 2026-09-12).
async function portalResendConfig(env = process.env) {
  const { supabaseRequest } = require("./sandbox"); // the schema switch (practice copy vs live), like every server client
  const url = String(env.SUPABASE_URL || "https://ptnzaeprvkgjgtupmcty.supabase.co").replace(/\/$/, "");
  const key = env.SUPABASE_SERVICE_ROLE_KEY || env.SUPABASE_SERVICE_KEY || "";
  if (!key) return null;
  const call = async (path, options = {}) => {
    const target = supabaseRequest(path, options.headers || {});
    const response = await fetch(`${url}${target.path}`, { ...options, headers: { apikey: key, Authorization: `Bearer ${key}`, "Content-Type": "application/json", ...target.headers } });
    return response.json().catch(() => null);
  };
  try {
    const rows = await call("/rest/v1/communications_settings?select=setting_key,secret_id,value&setting_key=in.(resend_api_key,resend_from_address)");
    const map = new Map((Array.isArray(rows) ? rows : []).map(r => [r.setting_key, r]));
    const from = String(map.get("resend_from_address")?.value?.value || "").trim();
    if (!map.get("resend_api_key")?.secret_id || !from) return null;
    const secret = await call("/rest/v1/rpc/communications_read_setting_secret", { method: "POST", body: JSON.stringify({ p_key: "resend_api_key" }) });
    const apiKey = typeof secret === "string" ? secret.trim() : "";
    return apiKey ? { key: apiKey, from: from.includes("<") ? from : `Lorenzo's Dog Training Team <${from}>`, ready: true, source: "portal" } : null;
  } catch { return null; }
}

async function officeResendConfig(env = process.env) {
  const fromEnv = resendConfig(env);
  if (fromEnv.ready) return fromEnv;
  return (await portalResendConfig(env)) || fromEnv;
}

module.exports = { DEFAULT_FROM, RESEND_URL, resendConfig, portalResendConfig, officeResendConfig, buildBookingEmail, buildRequestEmail, sendViaResend };
