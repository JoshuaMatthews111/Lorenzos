...
(function () {
  ...
  var FORM = ${safeForm};
  var PH = { address: "Street, city, state, ZIP", age: "e.g. 2 years", behavior: "What would you like help with?" };
  ...
  function question(fd, attr) {
    ...
    return "<label" + (full ? ' class="full"' : "") + ">" + esc(fd.label) + (fd.required ? ' <span class="req">*</span>' : "") + control(fd, attr) + "</label>";
  }
  ...
  function openForm() {
    ...
    if (!prefilled) {
      ...
      var l = leadInfo || {};
      ["first_name", "last_name", "phone", "email", "address"].forEach(function (k) { if (l[k] && f.elements[k]) f.elements[k].value = l[k]; });
