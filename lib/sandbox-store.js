...
const OBJECT_PATH = "sandbox/ops-log.json";
const MAX_OPS = 2000;
...
async function writeOps(ops) {
  ...
  const response = await fetch(storageUrl(), {
    method: "POST",
    headers: headers({ "Content-Type": "application/json", "x-upsert": "true", "Cache-Control": "no-store" }),
    body
  });
  if (!response.ok) throw new Error(`Sandbox op log could not be written (${response.status})`);
}
