...
const DEFAULT_PRACTICE_OPS_PHONE = "+12168168026"; // Tim (Operations), tester. Practice copy only (Joshua 2026-09-12).
...
const DEFAULT_PRACTICE_EMAIL_TO = "marketing@lorenzosdogtrainingteam.com"; // Joshua 2026-09-12: practice office emails go to marketing@ for now.
...
function normalizeSettings(value = {}) {
  ...
  return { errors, value: { recipients, practice_trainer_phone: practicePhone, practice_operations_phone: opsPhone, practice_email_to: emailOk(practiceEmail) ? practiceEmail : "" } };
}

function defaultSettings() {
  return { recipients: DEFAULT_RECIPIENTS.map(r => ({ ...r })), practice_trainer_phone: DEFAULT_PRACTICE_TRAINER_PHONE, practice_operations_phone: DEFAULT_PRACTICE_OPS_PHONE, practice_email_to: DEFAULT_PRACTICE_EMAIL_TO, saved: false };
}
...
async function sendOpsAlert(stage, fields = {}) {
  ...
  if (!isSandbox()) return { ...base, status: "skipped", reason: "Operations texts are switched on for the practice copy only." };
  const settings = await loadSettings().catch(() => null);
  const phone = e164(settings?.practice_operations_phone ?? DEFAULT_PRACTICE_OPS_PHONE);
  ...
  const result = await postHook(url, { stage, operations_phone: phone, ...fields });
...
async function sendLeadOfficeEmails(leadId, { settings = null, config = null } = {}) {
  ...
  for (const notice of notices) {
    ...
    const email = M.buildBookingEmail({ lead, booking: current, trainerName: current.trainer_name, staffLink: staffLeadLink(lead.id), lane, practice: isSandbox(), redirectedFrom, kind: notice.kind || "" });
...
async function afterBooking({ lead, booking, trainer, setting }) {
  ...
  try { slot = slotParts(booking?.slot_start, booking?.time_zone || setting?.time_zone || "America/New_York"); } catch { /* keep blanks */ }
...
async function enterPipeline(leadId, { via = "website-form", maxAgeMinutes = MAX_AGE_MINUTES } = {}) {
  ...
  const opsNew = await sendOpsAlert("new_lead", {
    ...
    source: clean(rawOf(won).source_page || via, 80),
...
module.exports = {
  ...
  mergePipelineRecord, afterBooking, afterOfficeRequest, enterPipeline, staffLeadLink,
