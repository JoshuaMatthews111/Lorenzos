...
function renderAdPage(rawContent, options = {}) {
  ...
  const render = {
    ...
    care: s => `    <section class="market-care-section section">
      <div class="container market-care-grid">
        ...
        ${s.photo ? `<figure class="market-care-photo"><img src="${escapeHtml(asset(s.photo))}" alt="${escapeHtml(s.photoAlt)}" style="${arStyle(s.photo)}object-position:${s.photoPos || "center"}" loading="lazy" decoding="async"></figure>` : ""}
      </div>
    </section>`,
    ...
    image: s => !s.photo ? "" : `    <section class="market-image-section section${s.size ? ` imgsz-${s.size}` : ""}">
      <div class="container">
        <figure class="market-embed-figure">
          ${s.link ? `<a href="${escapeHtml(s.link)}" target="_blank" rel="noopener">` : ""}<img src="${escapeHtml(asset(s.photo))}" alt="${escapeHtml(s.photoAlt || s.caption || "Photo")}" style="${arStyle(s.photo)}object-position:${s.photoPos || "center"}" loading="lazy" decoding="async">${s.link ? "</a>" : ""}
  ...
  const heroMedia = casePanel ? (hero.heroPhoto ? `<div class="market-hero-media market-hero-stack">
          ...
          })();<\/script>` : `<img src="${escapeHtml(asset(hero.photo))}" alt="${escapeHtml(hero.photoName)} — ${escapeHtml(content.market)}" style="${arStyle(hero.photo)}object-position:${hero.photoPos || "center"}" loading="eager" fetchpriority="high" decoding="async">`}
          <figcaption>
...
${BASE_STYLE.replace('url("assets/ldtt-dog-mark.png")', `url("${asset("assets/ldtt-dog-mark.png")}")`)}
  ...
  ${fontHead(content)}` : ""}${extraStyle}${editorStyle}
  ...
  <header class="ad-header ad-header-v2 market-header">
    <div class="container ad-nav ad-nav-v2">
      <a class="ad-brand-v2" href="#top" aria-label="Back to top">
        <img class="logo" src="${asset("assets/lorenzo-logo-white.png")}" alt="Lorenzo's Dog Training Team">
