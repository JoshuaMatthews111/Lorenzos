// Shared ad landing page template.
//
// ONE skeleton for every paid-ad market page: head, Google Ads tag, Meta pixel,
// hero + lead form, the sections below the fold, footer, scripts. It is used by
//   - scripts/generate-market-pages.mjs  (static files, written at build time)
//   - api/ad-page.js                      (Page Studio pages served from the
//                                          ad_pages table, no deploy needed)
//   - trainer-backoffice/page-studio.js   (the live canvas in the browser)
// so the three can never drift apart. Change the page here, and only here.
//
// The template renders a CONTENT object, never raw HTML from the office. Every
// text field goes through escapeHtml, every URL through safeUrl, every colour
// through safeColor, so a page saved from Page Studio cannot break the site.
//
// CommonJS + browser global on purpose (see lib/ad-page-markets.js). No Node
// built-ins are used; the caller passes an imageAspect() reader when it has one.
(function (root, factory) {
  const markets = typeof module !== "undefined" && module.exports
    ? require("./ad-page-markets.js")
    : root.LDTT_AD_PAGE_MARKETS;
  const mod = factory(markets);
  if (typeof module !== "undefined" && module.exports) module.exports = mod;
  else root.LDTT_AD_PAGE_TEMPLATE = mod;
})(typeof globalThis !== "undefined" ? globalThis : this, function (MARKETS) {
const { YEARS_IN_BUSINESS, markets, FLOWS, casePanels } = MARKETS;
...
// The page CSS, verbatim from the original generator. It is a plain string (no
// interpolation) so it can never be broken by page content.
const BASE_STYLE = `    .market-copy .ad-lead.market-hook{border-left:4px solid #c8102e;background:rgba(200,16,46,.07);padding:10px 14px;border-radius:0 10px 10px 0;font-size:1.02em}
    ...
    /* Image / embed sections + the per-section layout choices (Page Editor).
       secw-wide/-full override the shared .container width; imgsz-* caps the
       embedded photo. All opt-in per section, "" leaves the design untouched. */
    .market-image-section{padding:34px 0}
    .market-embed-figure{margin:0;text-align:center}
    .market-embed-figure img{width:100%;max-width:880px;height:auto;aspect-ratio:var(--ar,auto);object-fit:cover;border-radius:14px;display:block;margin:0 auto;box-shadow:0 18px 44px rgba(0,0,0,.28)}
    .market-embed-figure figcaption{margin-top:10px;font-size:.9rem;color:#5a6478}
    .market-image-section.imgsz-small .market-embed-figure img{max-width:420px}
    .market-image-section.imgsz-medium .market-embed-figure img{max-width:680px}
    .market-image-section.imgsz-large .market-embed-figure img{max-width:1050px}
    .market-image-section.imgsz-full .market-embed-figure img{max-width:100%}
    .market-landing .secw-wide .container{--max:1400px}
    .market-landing .secw-full .container{--max:100%;width:100%}
    .market-landing .secw-full.market-image-section .market-embed-figure img{border-radius:0;box-shadow:none}
...
// ---------------------------------------------------------------------------
// Curated choices the office can pick from. Anything not in these lists is
// refused by normalizeContent, so a stored page can only ever reference them.
// ---------------------------------------------------------------------------
const FONTS = [
  { id: "", label: "Site default (Inter)", family: "", stack: "" },
  { id: "inter", label: "Inter", family: "Inter", stack: `"Inter",Arial,sans-serif` },
  { id: "montserrat", label: "Montserrat", family: "Montserrat", stack: `"Montserrat","Inter",Arial,sans-serif` },
  { id: "poppins", label: "Poppins", family: "Poppins", stack: `"Poppins","Inter",Arial,sans-serif` },
  { id: "nunito", label: "Nunito", family: "Nunito", stack: `"Nunito","Inter",Arial,sans-serif` },
  { id: "raleway", label: "Raleway", family: "Raleway", stack: `"Raleway","Inter",Arial,sans-serif` },
  { id: "oswald", label: "Oswald", family: "Oswald", stack: `"Oswald","Inter",Arial,sans-serif` },
  { id: "bebas", label: "Bebas Neue", family: "Bebas Neue", stack: `"Bebas Neue","Oswald",Arial,sans-serif` },
  { id: "playfair", label: "Playfair Display", family: "Playfair Display", stack: `"Playfair Display",Georgia,serif` },
  { id: "lora", label: "Lora", family: "Lora", stack: `"Lora",Georgia,serif` },
  { id: "merriweather", label: "Merriweather", family: "Merriweather", stack: `"Merriweather",Georgia,serif` }
];
const FONT_BY_ID = Object.fromEntries(FONTS.map(font => [font.id, font]));

// Button & icon hover effects — pickable per page in the Page Editor's Style tab.
// "" (design default) emits nothing, so existing pages render byte-identical.
const HOVER_FX = [
  { id: "", label: "Design default" },
  { id: "lift", label: "Lift (rises with a shadow)" },
  { id: "grow", label: "Grow (gently enlarges)" },
  { id: "glow", label: "Glow (brightens with a ring)" },
  { id: "pulse", label: "Pulse (heartbeat while hovered)" },
  { id: "none", label: "Still (no movement at all)" }
];
const HOVER_FX_BY_ID = Object.fromEntries(HOVER_FX.map(fx => [fx.id, fx]));
// One selector group so every effect touches the same controls: CTA buttons,
// the price pill, the consult form's submit, and the guarantee icon chip.
const HOVER_FX_TARGETS = ['.btn', '.market-primary-cta', '.price-cta', '.price-pill', '.ad-consult-panel button[type="submit"]', '.visit', '.gcta']
  .map(sel => `body.market-landing ${sel}`).join(",");
function hoverFxCss(id) {
  if (!id || !HOVER_FX_BY_ID[id]) return "";
  const S = HOVER_FX_TARGETS;
  const H = HOVER_FX_TARGETS.split(",").map(sel => `${sel}:hover`).join(",");
  if (id === "none") return `${S},${H}{transition:none !important;transform:none !important;animation:none !important}`;
  const base = `@media (hover:hover){${S}{transition:transform .18s ease-out,box-shadow .18s ease-out,filter .18s ease-out;will-change:transform}`;
  const fx = {
    lift: `${H}{transform:translateY(-3px);box-shadow:0 12px 24px rgba(10,16,40,.28)}`,
    grow: `${H}{transform:scale(1.045)}`,
    glow: `${H}{filter:brightness(1.08);box-shadow:0 0 0 3px rgba(255,255,255,.28),0 10px 26px rgba(10,16,40,.30)}`,
    pulse: `@keyframes ldttHoverPulse{0%,100%{transform:scale(1)}50%{transform:scale(1.05)}}${H}{animation:ldttHoverPulse .9s ease-in-out infinite}`
  }[id];
  return `${base}${fx}}@media (prefers-reduced-motion:reduce){${S},${H}{transition:none;transform:none;animation:none}}`;
}

const ARCHES = [
  { id: "hq", label: "HQ — navy, facility banner" },
  { id: "portrait", label: "Portrait — parchment, wine" },
  { id: "metro", label: "Metro — midnight, giant type" },
  { id: "specialty", label: "Specialty — near-black, gold" },
  { id: "coast", label: "Coast — white, ocean blue" },
  { id: "mission", label: "Mission — red + gold" },
  { id: "heritage", label: "Heritage — hunter green, tan" },
  { id: "portrait2", label: "Portrait II — coral + palm" },
  { id: "resort", label: "Resort — aqua over sand" },
  { id: "campus", label: "Campus — white, blue + maize" }
];
const ARCH_IDS = new Set(ARCHES.map(a => a.id));

const CASE_PANELS = [
  { id: "", label: "None — show the photo" },
  { id: "dog-training-san-diego-ca", label: "Published pricing panel" },
  { id: "dog-training-san-antonio-tx", label: "Spanish-speaking + military panel" },
  { id: "dog-training-ann-arbor-mi", label: "Apartment puppy plan panel" }
];

const SECTION_TYPES = [
  { type: "proof", label: "Proof numbers", help: "The four big numbers under the hero." },
  { type: "path", label: "How it works", help: "The four training paths." },
  { type: "pricing", label: "Offer + guarantee", help: "Published prices (forward) or the 90-day guarantee only (quiet)." },
  { type: "care", label: "Story / trainer strip", help: "One headline, one paragraph, one photo." },
  { type: "testi", label: "Proof cards", help: "Three short reason cards." },
  { type: "trainers", label: "Local trainers", help: "Names the certified trainers for this market." },
  { type: "faq", label: "FAQ", help: "Questions and answers." },
  { type: "guide", label: "Free guide", help: "The free PDF opt-in form." },
  { type: "cta", label: "Closing call to action", help: "City reviews (when published) and the final button." },
  { type: "custom", label: "Custom block", help: "Your own headline, paragraph and button." },
  { type: "image", label: "Image / embed", help: "A photo anywhere on the page. Pick it, size it, widen it, caption it, link it." }
];
const SECTION_TYPE_SET = new Set(SECTION_TYPES.map(s => s.type));
...
const STATES = {
  ...
  TN: "Tennessee", TX: "Texas", UT: "Utah", VT: "Vermont", VA: "Virginia", WA: "Washington", WV: "West Virginia", WI: "Wisconsin", WY: "Wyoming", DC: "Washington DC"
};

// ---------------------------------------------------------------------------
// Sanitisers. Nothing the office types can escape these.
// ---------------------------------------------------------------------------
const text = (value, max = 600) => String(value ?? "").replace(/[\u0000-\u0008\u000B\u000C\u000E-\u001F\u007F]/g, "").trim().slice(0, max);
const longText = value => text(value, 4000);
const list = (value, max = 12, each = 200) => (Array.isArray(value) ? value : String(value ?? "").split("|"))
  .map(item => text(item, each)).filter(Boolean).slice(0, max);
const safeColor = (value, fallback) => (/^#[0-9a-f]{6}$/i.test(String(value || "").trim()) ? String(value).trim() : fallback);
const safeTint = (value, fallback) => (/^\d{1,3},\d{1,3},\d{1,3}$/.test(String(value || "").trim()) ? String(value).trim() : fallback);
// Relative asset paths, root paths and https URLs only. Nothing else can be
// dropped into a src or href.
const safeUrl = value => {
  const url = text(value, 500);
  if (!url) return "";
  if (/^https:\/\/[^\s"'<>]+$/i.test(url)) return url;
  if (/^\/?(assets|trainer-backoffice)\/[a-z0-9_\-./%]+$/i.test(url) && !url.includes("..")) return url;
  return "";
};
const safeSlug = value => text(value, 80).toLowerCase().replace(/[^a-z0-9-]+/g, "-").replace(/^-+|-+$/g, "").slice(0, 80);
const safePos = value => (/^[a-z0-9% ]{1,40}$/i.test(String(value || "").trim()) ? String(value).trim() : "");
const uid = prefix => `${prefix}-${Math.random().toString(36).slice(2, 8)}`;

// Photos + logo: size and move (portal chain step 5, DO-NOT-BREAK rule 76).
// Rule-68 pattern: numbers only, clamped to these limits, and a missing or
// empty value means the design's own. Nothing the office types reaches CSS
// except these integers, and an unchanged page stores and renders exactly
// what it did before (the keys are left out when empty).
//   photoW  = the photo's width in % of its frame (20-100)
//   photoX/Y = how far it is moved, in px (right/down positive)
//   logo.w  = the header logo's width in px; logo.x/y = moved, in px
const MEDIA_LIMITS = Object.freeze({ photoW: [20, 100], photoX: [-400, 400], photoY: [-300, 300], logoW: [60, 360], logoX: [-200, 200], logoY: [-40, 40] });
function clampInt(value, [min, max]) {
  if (value === "" || value === null || value === undefined || typeof value === "boolean") return null;
  const n = Math.round(Number(value));
  return Number.isFinite(n) ? Math.min(max, Math.max(min, n)) : null;
}
// The size/move keys of one photo, only the ones that differ from the design.
function photoLayout(src) {
  const out = {};
  const w = clampInt(src?.photoW, MEDIA_LIMITS.photoW);
  const x = clampInt(src?.photoX, MEDIA_LIMITS.photoX);
  const y = clampInt(src?.photoY, MEDIA_LIMITS.photoY);
  if (w !== null) out.photoW = w;
  if (x) out.photoX = x;
  if (y) out.photoY = y;
  return out;
}
// The header logo: its own picture (blank = the LDTT logo) + size + move.
function logoLayout(src) {
  const raw = src && typeof src === "object" ? src : {};
  const out = {};
  const photo = safeUrl(raw.photo);
  const w = clampInt(raw.w, MEDIA_LIMITS.logoW);
  const x = clampInt(raw.x, MEDIA_LIMITS.logoX);
  const y = clampInt(raw.y, MEDIA_LIMITS.logoY);
...
// CSS built from the clamped integers only. "" when nothing was changed.
// `translate` (not `transform`) so it never fights a design's own transform;
// the phone rule (mediaPhoneCss) drops it below 700px.
function photoStyle(item) {
  const m = photoLayout(item);
  return `${m.photoW ? `max-width:${m.photoW}%;margin-inline:auto;display:block;` : ""}${m.photoX || m.photoY ? `translate:${m.photoX || 0}px ${m.photoY || 0}px;` : ""}`;
}
function logoStyle(logo) {
  const m = logoLayout(logo) || {};
  return `${m.w ? `width:${m.w}px;height:auto;max-width:46vw;` : ""}${m.x || m.y ? `translate:${m.x || 0}px ${m.y || 0}px;` : ""}`;
}
const MEDIA_PHONE_CSS = `
  <style>@media (max-width:700px){img[style*="translate:"]{translate:none!important}}</style>`;

// ---------------------------------------------------------------------------
// Content model. marketToContent() turns one record from lib/ad-page-markets.js
// into the editable shape Page Studio works with; renderAdPage() turns that
// shape into the page. The static generator runs both, so a static page and a
// Page Studio page are the same thing at different times.
// ---------------------------------------------------------------------------
function defaultSection(type, market = {}) {
  const city = market.city || "your city";
  switch (type) {
    case "proof":
      return { type, stats: [
        { value: String(YEARS_IN_BUSINESS), label: "Years of experience" },
        { value: "100,000+", label: "Dogs trained of all breeds" },
        { value: "50+", label: "Professional trainers nationwide" },
        { value: market.proofValue || "Free", label: market.proofLabel || "Evaluation" }
      ] };
    case "path": {
      const forward = market.priceMode === "forward";
      return { type, eyebrow: "Training built for real life", heading: "A clearer path for your dog.", text: market.proof || `${city} families get local obedience and behavior help, personally coordinated through Lorenzo's office.`, services: [
        { title: "Dog Obedience Training", text: `Practical obedience that helps your dog listen in the moments that matter: at home, on walks, around people, and around distractions. In-home and private dog training options available.${forward ? " From $1,250." : ""}` },
        { title: "Behavior Modification", text: "Balanced training support for aggression, reactivity, barking, pulling, jumping, anxiety, and house-soiling with owner guidance built in." },
        { title: "Specialty & Service Dog Training", text: "Outcome-based support for protection, service and assistance needs, scent work, utility training, retrieval, and advanced control." },
        { title: "Board & Train", text: `Your dog trains with a professional and comes home with real skills — with owner handoff lessons included.${forward ? " From $2,500." : ""} Fall and holiday slots fill 6–8 weeks ahead.` }
      ] };
    }
    case "pricing":
      return { type, mode: market.priceMode === "forward" ? "forward" : "quiet" };
    case "care":
      return { type, eyebrow: market.care?.eyebrow || "Why families choose us", heading: market.care?.h2 || `Real training for real ${city} dogs.`, text: market.care?.p || "", button: "Book My Consultation", photo: market.photo2 || "", photoAlt: market.photo2Alt || "", photoPos: market.photo2Pos || "" };
    case "testi":
      return { type, eyebrow: "Trusted by dog owners", heading: market.care?.t || "Why it works.", items: (market.testis || [
        { h: "Free evaluation", p: "A professional reads your dog before anyone talks plans." },
        { h: "A clear plan", p: "You know what happens next, and why." },
        { h: "Guaranteed in writing", p: "The 90-day Limited Training Guarantee comes standard." }
      ]).map(item => ({ h: item.h, p: item.p })) };
    case "guide":
      return { type, eyebrow: "Free PDF Guide", heading: "Not ready to talk yet? Get the free guide.", bookTitle: "The 5-Step Calm Dog Blueprint", text: "gives owners a simple daily foundation for focus, obedience, and calmer behavior before problems get worse." };
    case "cta":
      return { type, heading: `Need dog training in ${market.market || city}?`, text: "Submit the quick request and let Lorenzo's office help with the next step.", button: "Book My Consultation" };
    case "image":
      return { type, photo: "", photoAlt: "", photoPos: "", caption: "", link: "" };
    case "trainers":
      return { type, eyebrow: "Your local team", heading: `Certified trainers serving ${city}.`, text: `Every Lorenzo's trainer is certified at our Cleveland headquarters and backed by the office, so your ${city} evaluation is handled by a professional, not a call center.` };
    case "faq":
      return { type, eyebrow: "Questions", heading: "Good questions. Straight answers.", items: [
    ...
    case "custom":
      return { type, eyebrow: "", heading: "Your headline here", text: "Your paragraph here.", button: "Book My Consultation" };
    default:
      return null;
  }
}

function blankSection(type, market = {}) {
  const section = defaultSection(type, market);
  return section ? { id: uid(type), ...section } : null;
}

function marketToContent(market) {
  ...
  return {
    ...
    title: market.title,
    description: "",
    market: market.market, city: market.city, state: market.state, area: market.area,
    trainers: market.trainers, nearby: [...market.nearby], zipCodes: [...(market.zipCodes || [])],
    spanish: Boolean(market.spanish), hq: market.slug === "dog-training-cleveland-oh",
    arch: market.arch, mk1: market.mk1, mk2: market.mk2, tint: market.tint, tint2: market.tint2,
    font: "", fontBody: "", accent: "", hoverFx: "", cover: "",
    hero: {
      word: market.word, h1: market.h1, hook: market.hook,
      benefits: market.benefits.split("|"), issues: market.issues.split("|"), checks: [...market.checks],
      priceMode: market.priceMode === "forward" ? "forward" : "quiet",
      photo: market.photo || "", photoName: market.photoName || "", photoCaption: market.photoCaption || "", photoPos: market.photoPos || "",
      heroPhoto: market.heroPhoto || "", heroVideo: market.heroVideo || "", heroPoster: market.heroPoster || "",
      casePanel: casePanels[market.slug] ? market.slug : "",
      resultsVideo: market.slug === "dog-training-cleveland-oh"
    },
    sections: flow.map((type, index) => ({ id: `${type}-${index + 1}`, ...defaultSection(type, market) }))
  };
}

// Deep-clone with every string field passed through the sanitiser. Unknown
// keys are dropped, so the stored JSON only ever holds what the page renders.
function normalizeContent(input) {
  const src = input && typeof input === "object" ? input : {};
  const hero = src.hero && typeof src.hero === "object" ? src.hero : {};
  const arch = ARCH_IDS.has(src.arch) ? src.arch : "coast";
  const out = {
    version: 1,
    slug: safeSlug(src.slug),
    title: text(src.title, 120),
    description: text(src.description, 300),
    market: text(src.market, 80), city: text(src.city, 60), state: text(src.state, 2).toUpperCase(), area: text(src.area, 300),
    trainers: text(src.trainers, 200), nearby: list(src.nearby, 8, 60), zipCodes: list(src.zipCodes, 20, 10),
    spanish: src.spanish === true, hq: src.hq === true,
    arch, mk1: safeColor(src.mk1, "#152569"), mk2: safeColor(src.mk2, "#C8102E"),
    tint: safeTint(src.tint, "21,37,105"), tint2: safeTint(src.tint2, "200,16,46"),
    font: FONT_BY_ID[src.font] ? src.font : "", fontBody: FONT_BY_ID[src.fontBody] ? src.fontBody : "",
    accent: src.accent ? safeColor(src.accent, "") : "",
    hoverFx: src.hoverFx && HOVER_FX_BY_ID[src.hoverFx] ? src.hoverFx : "",
    cover: src.cover ? safeUrl(src.cover) : "",
    hero: {
      word: text(hero.word, 24), h1: text(hero.h1, 160), hook: text(hero.hook, 400),
      benefits: list(hero.benefits, 6, 60), issues: list(hero.issues, 8, 40), checks: list(hero.checks, 6, 160),
      priceMode: hero.priceMode === "forward" ? "forward" : "quiet",
      photo: safeUrl(hero.photo), photoName: text(hero.photoName, 120), photoCaption: text(hero.photoCaption, 160), photoPos: safePos(hero.photoPos),
      heroPhoto: safeUrl(hero.heroPhoto), heroVideo: safeUrl(hero.heroVideo), heroPoster: safeUrl(hero.heroPoster),
      casePanel: casePanels[hero.casePanel] ? hero.casePanel : "",
      resultsVideo: hero.resultsVideo === true,
      ...photoLayout(hero) // rule 76: size + move, only when changed
    },
    sections: []
  };
  const logo = logoLayout(src.logo); // rule 76: own logo, size, move — left out when all empty
  if (logo) out.logo = logo;
  const seen = new Set();
  (Array.isArray(src.sections) ? src.sections : []).slice(0, 20).forEach(raw => {
    if (!raw || !SECTION_TYPE_SET.has(raw.type)) return;
    const base = defaultSection(raw.type, {});
    const section = { id: /^[a-z0-9-]{1,40}$/.test(String(raw.id || "")) && !seen.has(raw.id) ? raw.id : uid(raw.type), type: raw.type };
    seen.add(section.id);
    for (const key of Object.keys(base)) {
      if (key === "type") continue;
      const value = raw[key];
      if (key === "stats") section.stats = (Array.isArray(value) ? value : base.stats).slice(0, 4).map((s, i) => ({ value: text(s?.value, 20) || base.stats[i]?.value || "", label: text(s?.label, 60) || base.stats[i]?.label || "" }));
      else if (key === "services") section.services = (Array.isArray(value) && value.length ? value : base.services).slice(0, 6).map(s => ({ title: text(s?.title, 80), text: text(s?.text, 400) }));
      else if (key === "items") section.items = (Array.isArray(value) ? value : base.items).slice(0, 8).map(item => raw.type === "faq" ? { q: text(item?.q, 200), a: text(item?.a, 800) } : { h: text(item?.h, 80), p: text(item?.p, 400) });
      else if (key === "mode") section.mode = value === "forward" ? "forward" : "quiet";
      else if (key === "photo") section.photo = safeUrl(value);
      else if (key === "photoPos") section.photoPos = safePos(value);
      else if (key === "link") section.link = safeUrl(value);
      else section[key] = typeof value === "string" ? (key === "text" ? longText(value) : text(value, 300)) : String(base[key] ?? "");
    }
    // Layout choices any section may carry (whitelisted, "" = the design's own).
    section.width = raw.width === "wide" || raw.width === "full" ? raw.width : "";
    section.size = ["small", "medium", "large", "full"].includes(raw.size) ? raw.size : "";
    if (section.photo !== undefined) Object.assign(section, photoLayout(raw)); // rule 76: size + move, only when changed
    out.sections.push(section);
  });
  return out;
}

// Page Studio "generate": a brand-new market that is not in the source data yet.
// It starts from the template market's record (same sections, same design) and
// swaps the place names, so the new page matches the format of the live ones.
function contentForNewMarket({ city, state, templateSlug, slug } = {}) {
  const template = markets.find(m => m.slug === templateSlug) || markets.find(m => m.slug === "dog-training-columbus-oh") || markets[0];
  const cleanCity = text(city, 60);
  const st = text(state, 2).toUpperCase();
  const stateName = STATES[st] || st;
  const marketLabel = `${cleanCity}, ${st}`;
  const content = marketToContent(template);
  const swap = value => {
    if (typeof value !== "string") return value;
    let next = value;
    // Longest first so "Cleveland / Akron, OH" is replaced before "Cleveland".
    const pairs = [[template.market, marketLabel], [template.city, cleanCity]];
    if (template.title) pairs.push([template.title, `${cleanCity} Dog Training`]);
    pairs.sort((a, b) => b[0].length - a[0].length).forEach(([from, to]) => { if (from) next = next.split(from).join(to); });
    return next;
  };
  const walk = node => {
  ...
  Object.assign(next, {
    slug: safeSlug(slug) || safeSlug(`dog-training-${cleanCity}-${st}`),
    title: `${cleanCity} Dog Training`,
    market: marketLabel, city: cleanCity, state: st,
    area: `${cleanCity} and surrounding ${stateName} communities`,
    nearby: [cleanCity, `${cleanCity} area`, stateName].filter(Boolean),
    zipCodes: [],
    trainers: "Lorenzo's certified trainers",
    spanish: false, hq: false
  });
  next.hero.resultsVideo = false;
  next.hero.heroVideo = "";
  next.hero.heroPoster = "";
...
// Renderer
// ---------------------------------------------------------------------------
function fontHead(content) {
  const heading = FONT_BY_ID[content.font];
  const body = FONT_BY_ID[content.fontBody];
  const families = [...new Set([heading, body].filter(f => f && f.family).map(f => f.family))];
  if (!families.length && !content.accent && !content.hoverFx) return "";
  ...
  const rules = [];
  if (body?.stack) rules.push(`body.market-landing{font-family:${body.stack}}`);
  if (heading?.stack) rules.push(`body.market-landing h1,body.market-landing h2,body.market-landing h3,body.market-landing .market-word,body.market-landing .price-big,body.market-landing .g90-num{font-family:${heading.stack}}`);
  if (content.accent) rules.push(`body.market-landing .btn-red,body.market-landing .market-primary-cta,body.market-landing .ad-consult-panel button[type="submit"],body.market-landing .price-pill{background:${content.accent} !important;color:#fff !important}body.market-landing .market-hook{border-left-color:${content.accent}}body.market-landing .gcta span,body.market-landing .g90-day{color:${content.accent}}`);
  if (content.hoverFx) rules.push(hoverFxCss(content.hoverFx));
  return `${links}
  <style>${rules.join("")}</style>`;
}

function renderAdPage(rawContent, options = {}) {
  const content = normalizeContent(rawContent);
  const base = String(options.base || "");
  const publicPath = options.publicPath || `/${content.slug}`;
  const editor = options.editor === true;
  const imageAspect = typeof options.imageAspect === "function" ? options.imageAspect : () => null;
  const asset = path => (!path || /^(https?:)?\/\//i.test(path) || path.startsWith("/") ? path : `${base}${path}`);
  const hero = content.hero;
  const metaDescription = content.description || `Request ${content.market} dog training from Lorenzo's Dog Training Team. Obedience training, puppy training, dog behavior modification, and advanced programs with fast local follow-up.`;
  const nearby = content.nearby.map(place => `<span>${escapeHtml(place)}</span>`).join("");
  const zipCodes = content.zipCodes.map(zip => `<span>${escapeHtml(zip)}</span>`).join("");
  const arStyle = path => (imageAspect(path) ? `--ar:${imageAspect(path)};` : "");
  const pricingForward = `
  ...
  const render = {
    proof: s => `    <section class="ad-proof-band-v2">
      <div class="container ad-proof-grid-v2">
        ${s.stats.map(stat => `<div><strong>${escapeHtml(stat.value)}</strong><span>${escapeHtml(stat.label)}</span></div>`).join("\n        ")}
      </div>
    </section>`,
    path: s => `    <section id="market-services" class="section market-path-section">
      <div class="container market-section-grid">
        ...
        <div class="ad-service-grid-v2 market-service-grid">
          ${s.services.map((service, i) => `<article>
            <span>0${i + 1}</span>
    ...
    pricing: s => (s.mode === "forward" ? pricingForward : pricingQuiet),
    care: s => `    <section class="market-care-section section">
      <div class="container market-care-grid">
        ...
        <div class="market-care-copy">
          <p>${escapeHtml(s.text)}</p>
          <a class="btn btn-secondary" href="#consultation">${escapeHtml(s.button)}</a>
        </div>
        ${s.photo ? `<figure class="market-care-photo"><img src="${escapeHtml(asset(s.photo))}" alt="${escapeHtml(s.photoAlt)}" style="${arStyle(s.photo)}object-position:${s.photoPos || "center"}${photoStyle(s) ? `;${photoStyle(s)}` : ""}"${editor ? ` data-ps-media="${escapeHtml(s.id)}"` : ""} loading="lazy" decoding="async"></figure>` : ""}
      </div>
    </section>`,
    testi: s => `    <section class="market-testimonial-section section">
      <div class="container">
        ...
        <div class="market-testimonial-grid">
          ${s.items.map((t, i) => `<article><div class="market-proof-icon" aria-hidden="true">0${i + 1}</div><h3>${escapeHtml(t.h)}</h3><p>${escapeHtml(t.p)}</p></article>`).join("\n          ")}
        </div>
      </div>
    </section>`,
    guide: s => `    <section id="free-ebook" class="market-guide-section market-guide-strip section">
      <div class="container market-guide-card market-guide-mini">
        <div>
          <span class="eyebrow">${escapeHtml(s.eyebrow)}</span>
          <h2>${escapeHtml(s.heading)}</h2>
          <p><strong>${escapeHtml(s.bookTitle)}</strong> ${escapeHtml(s.text)}</p>
        </div>
    ...
    cta: s => `    <section class="section tight">
      <div class="container">
        ...
        <section class="ad-city-reviews" data-approved-market-reviews data-review-destination="${escapeHtml(content.slug)}" hidden>
          <h2>What ${escapeHtml(content.city)} families say</h2>
        ...
        <div class="cta-band ad-cta-v2">
          ...
          <a class="btn btn-red" href="#consultation">${escapeHtml(s.button)}</a>
        </div>
      </div>
    </section>`,
    trainers: s => `    <section class="market-care-section market-trainers-section section">
      <div class="container market-care-grid">
        <div>
          <span class="eyebrow">${escapeHtml(s.eyebrow)}</span>
          <h2>${escapeHtml(s.heading)}</h2>
        ...
        <div class="market-care-copy">
          <p class="market-trainer-names">${escapeHtml(content.trainers)}</p>
          <p>${escapeHtml(s.text)}</p>
          <a class="btn btn-secondary" href="#consultation">Book My Consultation</a>
        </div>
      </div>
    </section>`,
    faq: s => `    <section class="market-testimonial-section market-faq-section section">
      <div class="container">
        <div class="market-section-heading">
          <span class="eyebrow">${escapeHtml(s.eyebrow)}</span>
          <h2>${escapeHtml(s.heading)}</h2>
        </div>
        <div class="market-faq-list">
          ${s.items.map(item => `<details><summary>${escapeHtml(item.q)}</summary><p>${escapeHtml(item.a)}</p></details>`).join("\n          ")}
        </div>
      </div>
    </section>`,
    custom: s => `    <section class="market-care-section market-custom-section section">
      <div class="container market-care-grid">
        <div>
        ...
        <div class="market-care-copy">
          <p>${escapeHtml(s.text)}</p>
          ${s.button ? `<a class="btn btn-secondary" href="#consultation">${escapeHtml(s.button)}</a>` : ""}
        </div>
      </div>
    </section>`,
    image: s => !s.photo ? "" : `    <section class="market-image-section section${s.size ? ` imgsz-${s.size}` : ""}">
      <div class="container">
        <figure class="market-embed-figure">
          ${s.link ? `<a href="${escapeHtml(s.link)}" target="_blank" rel="noopener">` : ""}<img src="${escapeHtml(asset(s.photo))}" alt="${escapeHtml(s.photoAlt || s.caption || "Photo")}" style="${arStyle(s.photo)}object-position:${s.photoPos || "center"}${photoStyle(s) ? `;${photoStyle(s)}` : ""}"${editor ? ` data-ps-media="${escapeHtml(s.id)}"` : ""} loading="lazy" decoding="async">${s.link ? "</a>" : ""}
          ${s.caption ? `<figcaption>${escapeHtml(s.caption)}</figcaption>` : ""}
        </figure>
      </div>
    </section>`
  };
  const extraTypes = new Set(content.sections.map(s => s.type).filter(t => ["trainers", "faq", "custom"].includes(t)));
  const extraStyle = extraTypes.size ? `
  <style>
    .market-trainer-names{font-size:1.15em;font-weight:800;color:#fff}
    .market-faq-list{display:grid;gap:12px;max-width:820px;margin:0 auto}
    .market-faq-list details{background:rgba(255,255,255,.06);border:1px solid rgba(255,255,255,.16);border-radius:12px;padding:14px 18px}
    .market-faq-list summary{cursor:pointer;font-weight:800;color:#fff;font-size:1.05em;list-style:none;display:flex;justify-content:space-between;gap:12px}
    .market-faq-list summary::after{content:"+";font-weight:900;color:var(--mk2)}
    .market-faq-list details[open] summary::after{content:"\\2013"}
    .market-faq-list p{margin:10px 0 0;color:#dfe4f5}
  </style>` : "";
  const editorStyle = editor ? `
  <style>
    [data-ps-section]{position:relative;transition:outline-color .15s}
    [data-ps-section]:hover{outline:3px dashed rgba(216,15,53,.55);outline-offset:-3px;cursor:pointer}
    [data-ps-section].ps-selected{outline:4px solid #d80f35;outline-offset:-4px}
    [data-ps-section]::before{content:attr(data-ps-label);position:absolute;top:8px;left:8px;z-index:50;background:#d80f35;color:#fff;font:800 12px/1 Inter,Arial,sans-serif;letter-spacing:.08em;text-transform:uppercase;padding:6px 10px;border-radius:999px;opacity:0;pointer-events:none;transition:opacity .15s}
    [data-ps-section]:hover::before,[data-ps-section].ps-selected::before{opacity:1}
    [data-ps-media]{cursor:move;touch-action:none;user-select:none;-webkit-user-drag:none}
    [data-ps-media]:hover{outline:3px solid #0b6bff;outline-offset:3px}
    [data-ps-media].ps-media-active{outline:3px solid #d80f35;outline-offset:3px}
  </style>` : "";
  // rule 76: the phone rule ships only on a page where something was moved.
  const anyMoved = [content.hero, ...content.sections].some(item => item.photoX || item.photoY) || Boolean(content.logo?.x || content.logo?.y);
  const mediaPhone = anyMoved ? MEDIA_PHONE_CSS : "";
  const casePanel = hero.casePanel ? casePanels[hero.casePanel] : "";
  const heroMedia = casePanel ? (hero.heroPhoto ? `<div class="market-hero-media market-hero-stack">
          <figure class="market-hero-photo-with-panel">
            <img src="${escapeHtml(asset(hero.heroPhoto))}" alt="${escapeHtml(hero.photoName || content.market + " dog training")}"${imageAspect(hero.heroPhoto)} loading="eager" decoding="async">
          </figure>
          ${casePanel}
        </div>` : casePanel) : `<figure class="market-hero-media market-trainer-media">
          ${hero.heroVideo ? `<video class="market-hero-video" src="${escapeHtml(asset(hero.heroVideo))}" poster="${escapeHtml(asset(hero.heroPoster || ""))}" autoplay muted loop playsinline preload="auto" aria-label="${escapeHtml(hero.photoName)}"></video>
          ...
          })();<\/script>` : `<img src="${escapeHtml(asset(hero.photo))}" alt="${escapeHtml(hero.photoName)} — ${escapeHtml(content.market)}" style="${arStyle(hero.photo)}object-position:${hero.photoPos || "center"}${photoStyle(hero) ? `;${photoStyle(hero)}` : ""}"${editor ? ` data-ps-media="hero"` : ""} loading="eager" fetchpriority="high" decoding="async">`}
          <figcaption>
            <strong>${escapeHtml(hero.photoName)}</strong>
            <span>${escapeHtml(hero.photoCaption)}</span>
          </figcaption>
        </figure>`;
  const resultsVideo = hero.resultsVideo ? `<figure class="market-hero-media ad-hero-video-card">
          <div class="ad-video-frame">
            <button class="ad-video-cover" type="button" data-video-cover aria-label="Play real client results video">
              <img src="${asset("assets/video/ldtt-cleveland-ad-reel-poster.jpg")}" alt="Real client results video cover" loading="lazy" decoding="async">
              <span class="video-bubble">Real Client Results</span>
              <span class="video-play-mark" aria-hidden="true">&#9654;</span>
            </button>
            <video controls preload="metadata" poster="${asset("assets/video/ldtt-cleveland-ad-reel-poster.jpg")}" playsinline>
              <source src="${asset("assets/video/ldtt-cleveland-ad-reel.mp4")}" type="video/mp4">
            </video>
          </div>
          <figcaption>
  ...
  const sectionLabel = section => SECTION_TYPES.find(t => t.type === section.type)?.label || section.type;
  const body = content.sections.map(section => {
    let html = render[section.type] ? render[section.type](section) : "";
    if (!html) return "";
    // Per-section width choice: wide (1400px) or full (edge to edge).
    if (section.width) html = html.replace('<section class="', `<section class="secw-${section.width} `);
    return editor ? html.replace("<section ", `<section data-ps-section="${escapeHtml(section.id)}" data-ps-label="${escapeHtml(sectionLabel(section))}" `) : html;
  }).filter(Boolean).join("\n\n");
  const heroTag = editor ? `<section class="market-hero" data-ps-section="hero" data-ps-label="Hero + form">` : `<section class="market-hero">`;
  return `<!doctype html>
<html lang="en">
<head>
  <meta charset="utf-8">
  <meta name="viewport" content="width=device-width,initial-scale=1">
  <title>${escapeHtml(content.title)} | Lorenzo's Dog Training Team</title>
  <meta name="description" content="${escapeHtml(metaDescription)}">
  <link rel="canonical" href="${SITE_ORIGIN}${escapeHtml(publicPath)}">
  <link rel="icon" type="image/png" href="${asset("assets/ldtt-favicon.png")}">
  <link rel="apple-touch-icon" href="${asset("assets/ldtt-favicon.png")}">
  <link rel="stylesheet" href="${asset("styles.css")}?v=${cacheVersion}">
  <meta property="og:type" content="website">
  <meta property="og:title" content="${escapeHtml(content.title)} | Lorenzo's Dog Training Team">
  <meta property="og:description" content="${escapeHtml(metaDescription)}">
  <meta property="og:url" content="${SITE_ORIGIN}${escapeHtml(publicPath)}">
  <meta property="og:image" content="${SITE_ORIGIN}/assets/get-started-premium-hero.jpg">
  ...
  <script type="application/ld+json">${JSON.stringify({
    ...
    "@type": content.hq ? "LocalBusiness" : "ProfessionalService",
    name: `Lorenzo's Dog Training Team — ${content.market}`,
    url: `${SITE_ORIGIN}${publicPath}`,
    telephone: "+1-866-436-4959",
    priceRange: "$1,250+",
    ...(content.hq ? { address: { "@type": "PostalAddress", streetAddress: "4805 Orchard Rd", addressLocality: "Garfield Heights", addressRegion: "OH", addressCountry: "US" } } : {}),
    areaServed: content.nearby,
    description: metaDescription
  }).replace(/<\//g, "<\\/")}</script>
  ${googleAdsHead()}
  ${metaPixelHead()}
  <style>
${BASE_STYLE.replace('url("assets/ldtt-dog-mark.png")', `url("${asset("assets/ldtt-dog-mark.png")}")`)}
  </style>${fontHead(content) ? `
  ${fontHead(content)}` : ""}${extraStyle}${mediaPhone}${editorStyle}
<script defer src="/_vercel/insights/script.js"></script><script defer src="/_vercel/speed-insights/script.js"></script></head>
<body id="top" class="market-landing ad-landing ad-landing-v2 arch-${content.arch}" style="--mk1:${content.mk1};--mk2:${content.mk2}" data-self-designed="1" data-market="${escapeHtml(content.market)}" data-hero-title="${escapeHtml(hero.h1)}" data-issues="${escapeHtml(hero.issues.join("|"))}">
  <header class="ad-header ad-header-v2 market-header">
    <div class="container ad-nav ad-nav-v2">
      <a class="ad-brand-v2" href="#top" aria-label="Back to top">
        <img class="logo" src="${escapeHtml(asset(content.logo?.photo || "assets/lorenzo-logo-white.png"))}" alt="Lorenzo's Dog Training Team"${logoStyle(content.logo) ? ` style="${logoStyle(content.logo)}"` : ""}${editor ? ` data-ps-media="logo"` : ""}>
  ...
  <main>
    ${heroTag}
      <img class="market-hero-bg" src="${asset("assets/get-started-premium-hero.jpg")}" alt="" width="1680" height="945" decoding="async" aria-hidden="true">
      <div class="market-tint" aria-hidden="true" style="background:linear-gradient(135deg, rgba(${content.tint},.55), rgba(${content.tint2},.28))"></div>
      <div class="container market-hero-grid">
        <div class="market-copy">
          <span class="market-word" aria-hidden="true">${escapeHtml(hero.word)}</span>
          <a class="ad-rating-row" href="https://www.google.com/search?q=Lorenzo%27s+Dog+Training+Team+reviews" target="_blank" rel="noopener" aria-label="Read Lorenzo's Dog Training Team Google reviews">
            <span>★★★★★</span>
            <strong>600+ Google reviews · ${escapeHtml(content.market)}</strong>
          </a>
          <h1>${escapeHtml(hero.h1)}</h1>
          <p class="ad-lead market-hook"><strong>${escapeHtml(hero.hook)}</strong></p>
          ...
          ${content.spanish ? `<p class="ad-lead market-spanish"><strong>Spanish-speaking trainers.</strong> <span lang="es">Nuestros entrenadores en ${escapeHtml(content.city)} hablan español — sus sesiones de entrenamiento pueden ser en español.</span> Please send your request in English so our office can route it quickly.</p>` : ""}
          <div class="ad-benefit-row">
            ${hero.priceMode === "forward" ? `<span class="price-pill">Training from $1,250</span>` : ""}
            ${hero.benefits.map(b => `<span>${escapeHtml(b)}</span>`).join("\n            ")}
          </div>
          <div class="ad-issue-strip" aria-label="Common dog training problems">
            ${hero.issues.map(i => `<span>${escapeHtml(i)}</span>`).join("")}
          </div>
          <ul class="ad-checks ad-checks-v2">
            ${hero.checks.map(c => `<li>${escapeHtml(c)}</li>`).join("\n            ")}
          </ul>
        ...
        <aside id="consultation" class="ad-consult-panel market-consult-panel">
          <div class="ad-consult-header">
            <span>${escapeHtml(content.market)}</span>
          ...
          <form class="ad-form-card ad-form-card-v2 contact-intake"
            ...
            <label class="consent-row">
              ...
              <span>By checking this box, I agree to receive text messages from Lorenzo's Dog Training Team about my request: follow-up on my inquiry, scheduling and confirming my free consultation or evaluation, and appointment reminders. Messages may be sent via autodialer. Consent is not a condition of any purchase or services. Message frequency varies. Message and data rates may apply. Reply STOP to unsubscribe and HELP for help. I also agree to the <a href="${asset("terms.html")}">Terms of Service</a> and <a href="${asset("privacy-policy.html")}">Privacy Policy</a>.</span>
            </label>
            ...
            <input type="hidden" name="trainer_name" value="${escapeHtml(content.trainers)}">
            <input type="hidden" name="assigned_trainer" value="${escapeHtml(content.trainers)}">
            <input type="hidden" name="trainer_market" value="${escapeHtml(content.market)}">
            <input type="hidden" name="ad_market" value="${escapeHtml(content.market)}">
            <input type="hidden" name="market_city" value="${escapeHtml(content.city)}">
            <input type="hidden" name="market_state" value="${escapeHtml(content.state)}">
            <input type="hidden" name="market_trainers" value="${escapeHtml(content.trainers)}">
            ...
            <input type="hidden" name="city" value="${escapeHtml(content.city)}">
            <input type="hidden" name="state" value="${escapeHtml(content.state)}">
            ...
            <input type="hidden" name="internal_route_note" value="Market ad landing page lead for ${escapeHtml(content.market)}. Office to confirm full address and evaluation preference during follow-up. Nearby trainer group: ${escapeHtml(content.trainers)}.${content.spanish ? " SPANISH MARKET: lead may arrive in Spanish - Spanish-speaking trainer available in this market; office should translate before routing." : ""}">
            ${content.spanish ? `<input type="hidden" name="spanish_market" value="yes">` : ""}
            <input type="hidden" name="source_page" value="${escapeHtml(content.slug)}">
            ${attributionInputs()}
  ...
  <script src="${asset("supabase-config.js")}"></script>
  <script src="${asset("script.js")}?v=${cacheVersion}"></script>
  <script src="${asset("market-landing.js")}?v=${cacheVersion}"></script>
  <script src="${asset("ad-funnel.js")}?v=${cacheVersion}"></script>
</body>
</html>`;
}

// ---------------------------------------------------------------------------
// Publish checklist. Runs in the browser before the button is enabled and again
// on the server before anything is written. Every item is a plain sentence the
// office can act on. `html` may be passed in by tests; normally it is rendered.
// ---------------------------------------------------------------------------
function publishChecklist(rawContent, options = {}) {
  const content = normalizeContent(rawContent);
  const html = typeof options.html === "string" ? options.html : renderAdPage(content, options);
  const checks = [];
  const add = (ok, label, fix) => checks.push({ ok: Boolean(ok), label, fix: ok ? "" : fix });
  add(/^[a-z0-9-]{3,80}$/.test(content.slug) && content.slug.startsWith("dog-training-"), "Web address is set", "Give the page a web address that starts with dog-training-, like dog-training-toledo-oh.");
  add(content.title, "Page title is filled in", "Type a page title in Page settings.");
  add(content.market && content.city && /^[A-Z]{2}$/.test(content.state), "Market, city and state are filled in", "Fill in the market name, city and two-letter state.");
  add(content.hero.h1, "Headline (H1) is filled in", "Type the big headline in the Hero section.");
  add(/<h1>[^<]+<\/h1>/.test(html), "Headline renders on the page", "The headline is empty after cleaning. Type plain words.");
  add(/class="[^"]*contact-intake[^"]*"/.test(html) && /name="phone"/.test(html), "Lead form is on the page", "The consultation form is missing. This should never happen; contact support.");
  add(html.includes(`fbq('init', '${metaPixelId}')`) && html.includes("connect.facebook.net"), "Meta pixel is on the page", "The Meta pixel is missing. This should never happen; contact support.");
  add(html.includes(googleAdsId), "Google Ads tag is on the page", "The Google Ads tag is missing. This should never happen; contact support.");
  add(content.sections.length > 0, "At least one section below the hero", "Add a section, for example Proof numbers or the closing call to action.");
  add(content.sections.some(s => s.type === "cta"), "A closing call to action exists", "Add the Closing call to action section so the page ends with a button.");
  content.sections.forEach((section, index) => {
    const where = `Section ${index + 1} (${SECTION_TYPES.find(t => t.type === section.type)?.label || section.type})`;
    if (["care", "testi", "cta", "faq", "trainers", "custom", "path", "guide"].includes(section.type)) add(section.heading, `${where} has a heading`, `${where}: type a heading.`);
    if (["care", "custom", "trainers", "path"].includes(section.type)) add(section.text, `${where} has text`, `${where}: type the paragraph.`);
    if (section.type === "testi" || section.type === "faq") {
      const filled = (section.items || []).filter(item => (item.h || item.q) && (item.p || item.a));
      add(filled.length >= 1 && filled.length === (section.items || []).length, `${where} items are complete`, `${where}: every card needs both its lines filled in, or remove the empty card.`);
    }
    if (section.type === "proof") add(section.stats.every(s => s.value && s.label), `${where} numbers are complete`, `${where}: every number needs a value and a label.`);
    if (section.type === "path") add(section.services.every(s => s.title && s.text), `${where} paths are complete`, `${where}: every path needs a title and a description.`);
  });
  return { ok: checks.every(c => c.ok), checks, failures: checks.filter(c => !c.ok) };
}

return {
  cacheVersion, googleEndpoint, googleAdsId, metaPixelId, attributionFields, SITE_ORIGIN,
  escapeHtml, attributionInputs, metaPixelHead, googleAdsHead, conversionAndAttributionScript,
  FONTS, HOVER_FX, ARCHES, CASE_PANELS, SECTION_TYPES, STATES, markets, FLOWS,
  defaultSection, blankSection, marketToContent, normalizeContent, contentForNewMarket,
  renderAdPage, publishChecklist, safeUrl, safeColor, safeSlug,
  MEDIA_LIMITS, photoLayout, logoLayout, photoStyle, logoStyle // rule 76
};
});
