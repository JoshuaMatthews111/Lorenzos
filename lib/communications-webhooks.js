...
async function beginWebhook(req, provider, settingKey, verifier = signatureMatches) {
  ...
  const valid = verifier === svixSignatureMatches
    ? verifier(req.headers, secret, payload)
    : verifier(header, secret, payload);
  if (!valid) return { error: "Webhook signature was rejected.", status: 401 };
  let data = {};
  ...
  try {
    ...
    if (error.status === 409) return { duplicate: true, data, eventId, eventType };
    throw error;
  }
  return { data, eventId, eventType };
}
