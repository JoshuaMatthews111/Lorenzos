...
// Vetting 2026-09-12: trainer rows spell the state both ways ("Cleveland, OH", "Streetsboro, Ohio"); the booking
// page shows one style, the postal code. Only a trailing full state name is changed.
...
function shortState(label) {
  return String(label || "").replace(/,\s*([A-Za-z .]+)$/, (whole, name) => { const code = US_STATE_CODES[name.trim().toLowerCase()]; return code ? `, ${code}` : whole; });
}

function marketLabel(trainer) { return shortState(marketLabelRaw(trainer)); }
function marketLabelRaw(trainer) {
