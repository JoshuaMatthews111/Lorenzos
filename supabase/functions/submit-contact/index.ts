...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // Meta Conversions API: the browser pixel is blocked for roughly half of
    // real visitors, so send the same event from here. Browser and server share
    // one event_id, so Meta de-duplicates. QA rows never reach Meta
    // (DO-NOT-BREAK 13), and this can never fail the form: the lead is already
    ...
    if (lead?.id && metaAllowed) {
      ...
      console.log("meta_capi_skipped", schema === "practice" ? "practice_copy" : "qa_row");
    }
